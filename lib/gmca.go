...
func signCert(req signer.SignRequest, ca *CA) (cert []byte, err error) {
	...
	if err != nil || cert == nil {
		log.Infof("GMSM template = %v\n cert = %v\n Type = %T\n", template, cert, template.PublicKey)
		log.Warning("gm.CreateCertificateToMem failed.")
		return nil, errors.Wrap(err, "cert is nil from CreateCertificateToMem.")
	}

	clientCert, err := sm2.ReadCertificateFromMem(cert)
	log.Info("==================== Exit ParseCertificate")
	if err != nil {
		log.Warning("sm2.ReadCertificateFromMem err:", err)
	} else {
	...
	if err != nil {
		log.Info("=====================error InsertCertificate!", err)
	}
